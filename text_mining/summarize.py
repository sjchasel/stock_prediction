...
def split_sentence(text, punctuation_list='!?。！？'):
    ...
    """
    text = ''.join(split_paragraph(text))
    sentence_set = []
...
def get_sum(text, ratio=0.3):
    """
    ratio是要抽取百分之多少的信息，返回摘要句子列表
    """
    stop_word = stopwords
    ...
    sort_sent_weight = ranking_base_on_weigth(sentence_with_words_weight,
                                              sentence_with_position_weight,
                                              sentence_score, feature_weight=[1, 1, 1])
    summarization = get_summarization(sentence_with_index, sort_sent_weight, topK_ratio=ratio)
    return summarization
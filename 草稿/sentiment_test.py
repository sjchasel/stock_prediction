...
class Analysis:
    ...
    def cutwords_jieba(self, sentence, userdict=PATH+'userdict.txt', stopwords=PATH+'stop1205.txt'):
        stropw = []
        if userdict:
    ...
    def sentiment_init(self):
        # 情感词典
        self.posdict = self.deal_wrap(PATH+'positive.txt')
        self.negdict = self.deal_wrap(PATH+'negative.txt')
        # 程度副词词典
        self.mostdict = self.deal_wrap(PATH+'most.txt')  # 权值为2
        self.verydict = self.deal_wrap(PATH+'very.txt')  # 权值为1.5
        self.moredict = self.deal_wrap(PATH+'more.txt')  # 权值为1.25
        self.ishdict = self.deal_wrap(PATH+'ish.txt')  # 权值为0.5
        self.insufficientdict = self.deal_wrap(PATH+'insufficiently.txt')  # 权值为0.25
        self.inversedict = self.deal_wrap(PATH+'inverse.txt')  # 权值为-1
...
for tempstr in a.deal_wrap(PATH+'data.txt'):
